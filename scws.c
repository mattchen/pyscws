...
static void _scws_ssegment(scws_t s, int end)
{
	...
	/* È¡³öµ¥´Ê¼°±êµã. Êý×ÖÔÊÐíÒ»¸öµãÇÒÏÂÒ»¸öÎªÊý×Ö,²»Á¬ÐøµÄ. ×ÖÄ¸ÔÊÐíÒ»¸ö²»Á¬ÐøµÄ' */
	while (start < end)
	{
		ch = txt[start++];
		...
		if (SCWS_IS_ALNUM(ch))
		{
			...
			while (start < end)
			{
				...
				{
					/* hightman.110419: - ³öÏÖÔÚ×ÖÄ¸ÖÐ¼äÔÊÐíÁ¬½Ó(0x2d), _ ÔÊÐíÁ¬½Ó(0x5f) */
...
static void _scws_mset_word(scws_t s, int i, int j)
{
	...
	item = s->wmap[i][j];
	/* hightman.070705: ¼ÓÈë item == null ÅÐ¶Ï, ·ÀÖ¹³¬³¤´Ê(255×ÖÒÔÉÏ)unsigned charÒç³ö */
	if ((item == NULL) || ((s->mode & SCWS_IGN_SYMBOL) 
      && !SCWS_IS_ECHAR(item->flag) && !memcmp(item->attr, attr_un, 2)))
		return;

	/* hightman.070701: É¢×Ö×Ô¶¯¶þÔª¾ÛºÏ */
...
static void _scws_msegment(scws_t s, int end, int zlen)
{
	...
	for (i = 0; i < zlen; i++)
	{
		...
		{			
			...
			// ×¢ÒâÔ­À´2×ÖÈËÃû,Ê¶±ðºóÈÔÎª2×ÖµÄÇé¿ö
...
scws_res_t scws_get_result(scws_t s)
{
	...
	while ((off = (off+clen)) < len)
	{
		...
		else if ((pflag & PFLAG_WITH_MB) && clen == 1)
		{
			...
			// mb + single-byte. allowd: alpha+num + ÖÐÎÄ
			if (!SCWS_IS_ALNUM(ch))
				break;
	...
	else
	{
		...
		else
		{
			...
			/* hightman.090523: Îª×ÖÄ¸Êý×Ö»ìºÏÔÙ¶È²ð½â, ´¿Êý×Ö, (>1 ? ´¿×ÖÄ¸ : Êý×Ö+×ÖÄ¸) */
...
char *scws_segment(char *text)
{
	...
	while (res = cur = scws_get_result(s))
	{
		while (cur != NULL)
		{
			...
			//retval = strncpy(retval, text+cur->off, cur->len);
			if (strcmp(cur->attr, "un") != 0)
			{
				if (strlen(retval) > 0)
				{
					sprintf(retval, "%s %.*s", retval, cur->len, text + cur->off);
				}
				else
				{
...
/* Python Wrapper */
static PyObject *
pyscws_segment(PyObject *self, PyObject *args)
{
	char *orig_str;
	...
	/*char *p;
	p = (char *) malloc(strlen(orig_str) * 2 + 1);
	p = strcpy(p, orig_str);*/
	//printf("args is: %s, strlen is: %d\n", orig_str, strlen(orig_str));
	
	char *stext = scws_segment(orig_str);
	//printf("stext is: %s, strlen is: %d\n", stext, strlen(stext));
	
	return (PyObject*)Py_BuildValue("s", stext);
}

static PyMethodDef
pyscwsMethods[] = {
	{ "segment", pyscws_segment, METH_VARARGS, "Python wrapped scws Segmentation Method." },
	{ NULL, NULL, 0, NULL },
};

PyMODINIT_FUNC initpyscws()
{
	Py_InitModule("pyscws", pyscwsMethods);
}
